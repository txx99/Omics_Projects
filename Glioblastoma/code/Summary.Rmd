...
```{r dea}
fit <- lmFit(expr_data, design)              # Linear model for large data sets
contrast_matrix <- makeContrasts(psPD - PD, levels=design) 
fit2 <- contrasts.fit(fit, contrast_matrix)  #statistics output
fit2 <- eBayes(fit2)                         #adjustment
```
...
number = Inf (infinity) tells it to list all the genes possible. 

adjust.method= specifies the method for adjusting p-value. BH adjustment compares each p-value to a critical value/threshold and accounts for multiple testing error (increasing likeliness of erroneous correlation due to large number of samples).
```{r results_extraction}
results <- topTable(fit2, number=Inf, adjust.method="BH")                   # top genes from fit2, all of them
sig_genes <- results[results$adj.P.Val < 0.05 & abs(results$logFC) > 1, ]   # filter by significance thresholds
```
Finally, we index the results data frame for only the genes with expression changes that meet specified significance criteria:

adjusted p-value < 0.05 and absolute of log fold-change greater than 1. 

We look at absolute fold-change values because the log fold-change is positive for upregulation and negative for downregulation, and we care about both.

## 7. Save results.
We can now save the DEA results.
```{r export_results}
...
The enhanced volcano plot shows the overall distribution of gene expression, with red dots being genes that meet the significance criteria.

The right side shows the up-regulated genes (in red) that have a log fold change of >1; where these genes are upregulated in psPD when compared to PD.

Whereas the left side shows the down regulated genes that have a log fold change of < -1; where these genes are down-regulated in psPD when compared to PD.

Both up-regulated and down-regulated genes are important to observe to aid in the understanding of the difference in gene expression when comparing the pseudo PD to actual PD in patients. This helps us develop patterns of recognition of psPD cases based on the expression of the gene.
